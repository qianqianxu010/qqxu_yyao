...
tt1=[detected(floor((j-3)/2),1),detected(floor((j-1)/2),1),detected(floor((j+1)/2),1)]; %middle three
tt2=[detected(end-2,1),detected(end-1,1),detected(end,1)]; % bottom three


%%%%Draw path
dd=preference_detected(1:10); % top 10 personalized-ranking annotators
aa=[tt1 tt2];
temptotal=[dd aa];
...
tt3=[detected(end-4,1),detected(end-3,1),detected(end-2,1),detected(end-1,1),detected(end,1)]; %bottom 5 position-biased annotators
%%%%%Draw path
...
ylabel('\delta','fontsize',20); set(gca, 'FontSize', 16); hold on;
yl=ylim;
plot([t(k),t(k)],[yl(1), yl(2)],'r:');text(t(k)+0.02,yl(1),'t_{cv}','fontsize',12,'color','r');hold on;%%%%%%%%%%%%%%%%%%%%%%%%%%%%%model 1 position_bias
